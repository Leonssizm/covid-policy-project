...
function validateInputs() {
    ...
    if(!isFilled(nameInputElementValue) || !lengthIsLonger(nameInputElementValue, 2)){
        formIsValid = false;
        setError(nameInputElement, 'სახელის ველი უნდა შედგებოდეს მინიმუმ 3 სიმბოლოსგან')
    }
    else if (lengthIsLonger(nameInputElementValue, 255)) {
        formIsValid = false;
        setError(nameInputElement, 'სახელის ველი უნდა შედგებოდეს მაქსიმუმ 255 სიმბოლოსგან')
    }
    else if(!REGEX.engLetters.test(nameInputElementValue)) {
        formIsValid = false;
        setError(nameInputElement, 'სახელის ველი უნდა შეიცავდეს მხოლოდ ანბანის ასოებს')
    }
    else{
        setSuccess(nameInputElement)
    }

    if(!isFilled(lastnameInputElementValue) || !lengthIsLonger(lastnameInputElementValue, 2)) {
        formIsValid = false;
        setError(lastnameInputElement, 'გვარის ველი უნდა შედგებოდეს მინიმუმ 3 სიმბოლოსგან')
    }
    else if (lengthIsLonger(nameInputElementValue, 255)) {
        formIsValid = false;
        setError(nameInputElement, 'გვარის ველი უნდა შედგებოდეს მაქსიმუმ 255 სიმბოლოსგან')
    }
    else if(!REGEX.engLetters.test(nameInputElementValue)) {
        formIsValid = false;
        setError(nameInputElement, 'გვარის ველი უნდა შეიცავდეს მხოლოდ ანბანის ასოებს')
    }
    else {
    ...
    if(!REGEX.EMAIL.test(emailInputElementValue)) {
        ...
        formIsValid = false;
        setError(emailInputElement, 'გთხოვთ დარეგისტრირდეთ Redberry-ს მეილით (youremail@redberry.ge)')
    }
    else {
